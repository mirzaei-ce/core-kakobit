...
###   This script attempts to download the signature file SHA256SUMS.asc from kakobit.org
...
WORKINGDIR="/tmp/kakobit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://kakobit.org/bin/"
VERSIONPREFIX="kakobit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'kakobit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Kakobit Core binary release signing key installed?"
   fi
